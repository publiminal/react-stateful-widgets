...
export default function Programmers() {
  ...
  const [devList, setDevsList] = useState(listOfAwesome)
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: idDev !== null ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          devList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setIdDev(dev.id)  /* in here set the featured id to be dev.id */ }>Feature</button>
            </div>
          )
        }
      </div>
